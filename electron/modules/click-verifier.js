  ...
  try {
    ...
    try {
      ...
      if (currentUrl !== initialUrl) {
        logToUI(`[Task ${taskId}] Click successful - URL changed from ${initialUrl} to ${currentUrl}`);
        return true;
      }
    } catch (urlError) {
      // If we can't get the URL, it likely means the page has navigated
      logToUI(`[Task ${taskId}] Click successful - page navigation detected (URL check failed)`);
      return true;
    }
    ...
    try {
      ...
      if (pageReloaded) {
        logToUI(`[Task ${taskId}] Click successful - page was reloaded`);
        return true;
      }

      if (navigationOccurred) {
        logToUI(`[Task ${taskId}] Click successful - navigation event detected`);
        return true;
      }
    } catch (evalError) {
      // If evaluation fails, it might mean the page has navigated
      logToUI(`[Task ${taskId}] Click successful - page navigation detected (evaluation failed)`);
      return true;
    }
    ...
    try {
      ...
      if (finalUrl !== initialUrl) {
        logToUI(`[Task ${taskId}] Click successful - navigation completed to ${finalUrl}`);
        return true;
      }
    } catch (navError) {
      // If navigation check fails, it might still be a successful click
      logToUI(`[Task ${taskId}] Click verification inconclusive - navigation check failed`);
    }

    logToUI(`[Task ${taskId}] Click verification failed - no navigation or new window detected`);
    return false;
  } catch (error) {
    // If we get here, it likely means the page has navigated or context was destroyed
    logToUI(`[Task ${taskId}] Click successful - page navigation detected (context destroyed)`);
    return true;
  }
}
